...
The Vantyx Bank backend can use Celcoin as the external Pix cash-out provider.
...
CELCOIN_DEBIT_NAME=Vantyx Bank
CELCOIN_PAYER_ID=replace-with-payer-document-digits
```

When the transfer is made by Pix key, Vantyx Bank first calls Celcoin DICT
(`/pix/v1/dict/v2/key`) and then submits the Pix payment using the returned
`endToEndId`. When the transfer is made by manual bank data, Vantyx Bank submits the
